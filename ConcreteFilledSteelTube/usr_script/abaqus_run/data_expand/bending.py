...
expand_pure_bending_data.reset_index(drop=True, inplace=True)
...
for section_type, section_height, section_width, thickness, fy, fc in zip(section_type_series, section_height_series,
    ...
    task_dir = os.path.join(CWD, 'run_files_expand', "bending",
                            f"expand_{expand_index}")
    # check if .odb file exist in task_dir
    # if os.path.exists(os.path.join(task_dir, "curve_1.txt")):
    if expand_index != 20:
        expand_index += 1
        continue
