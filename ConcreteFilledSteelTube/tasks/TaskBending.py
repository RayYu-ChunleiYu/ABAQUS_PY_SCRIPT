...
class TaskBending(Task):
    ...
    def script_refactor_common(self):
        ...
        load_support_location = self.parameters['load_support_location']
        ...
        self.script_FEM_run_string = self.script_FEM_run_string.replace("height = 1200",
                                                                        "height = {}".format(geometry['height']))
        self.script_FEM_run_string = self.script_FEM_run_string.replace("supprot_location_proportion = 1.0/10",
                                                                        "supprot_location_proportion = {}".format(
                                                                            load_support_location['support']))
        self.script_FEM_run_string = self.script_FEM_run_string.replace("load_location_proportion = 0.33",
                                                                        "load_location_proportion = {}".format(
                                                                            load_support_location['load']))
        self.script_FEM_run_string = self.script_FEM_run_string.replace("load_displacement_target = height/30",
    ...
    def run_fem_get_data_plot(self, show: bool = False):
        ...
        ax1 = fig.subplots()
        ax2 = ax1.twinx()
        ax1.plot(dataframe['displacement'], dataframe['force_1'], label="curve_1")
        ax1.plot(dataframe['displacement'], dataframe['force_2'], label='curve_2')
    ...
    def get_fu_as_excel(self):
        ...
        for dir in dirs:
            dir_abs = os.path.join(task_work_dir_abs, dir)
            if os.path.isdir(dir_abs):
                specimen_name = os.path.basename(dir_abs)
                excel_path = os.path.join(task_work_dir_abs, dir, 'data.xlsx')
                parameter_path = os.path.join(task_work_dir_abs, dir, 'parameter.json')
                ...
                current_data.insert(0, 'load_location', parameters['load_support_location']['load'])
                current_data.insert(0, 'support_location', parameters['load_support_location']['support'])
                ...
                interpolate_force_1 = current_data['force_1'][target_index - 1:target_index + 2]
                interpolate_force_2 = current_data['force_2'][target_index - 1:target_index + 2]
                interpolate_displacement = current_data['displacement'][target_index - 1:target_index + 2]

                force_1 = np.interp(current_limit, interpolate_strain, interpolate_force_1)
                force_2 = np.interp(current_limit, interpolate_strain, interpolate_force_2)
                ...
                target_data.loc["interpolate force 1"] = force_1
                target_data.loc["interpolate force 2"] = force_2
        ...
        force_1 = final_dataframe['force_1']
        force_1_interpolate = final_dataframe['interpolate force 1']
        force_2 = final_dataframe['force_2']
        force_2_interpolate = final_dataframe['interpolate force 2']
        support_location = final_dataframe['support_location']
        prue_span = height * (1 - 2 * support_location)
        moment_1 = -force_1 * prue_span / 3 / 1000000
        moment_2 = -force_2 * prue_span / 3 / 1000000
        moment_1_interpolate = -force_1_interpolate * prue_span / 3 / 1000000
        moment_2_interpolate = -force_2_interpolate * prue_span / 3 / 1000000
        final_dataframe.insert(0, 'moment_1', moment_1)
        final_dataframe.insert(0, 'moment_2', moment_2)
        final_dataframe.insert(0, 'moment_1_interpolate', moment_1_interpolate)
        final_dataframe.insert(0, 'moment_2_interpolate', moment_2_interpolate)
        final_dataframe.to_excel(os.path.join(task_work_dir_abs, 'final_data.xlsx'), index=False)

    def get_data_parse(self):
        controller_work_directory = self.parameters['controller_work_dir']
        task_work_directory = self.parameters['task_work_dir']
        curve1_path = os.path.join(controller_work_directory, task_work_directory, 'curve_1.txt')
        curve2_path = os.path.join(controller_work_directory, task_work_directory, 'curve_2.txt')
        ...
        with open(curve2_path, "r") as f:
            curve2_string = f.readlines()
        ...
        curve2_array = self.parse_string_to_array(curve2_string)
        ...
        curve_dataframe_dict = {
            ...
            "force_1": curve1_array[:, 1],
            "force_2": curve2_array[:, 1],
